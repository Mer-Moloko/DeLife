...
class App:
    def __init__(self, root):
        ...
        # --- new fields for metadata-encryption settings ---
        # if True, encrypt metadata by default (requires password to be set)
        self.always_encrypt_var = tk.BooleanVar(value=False)
        # stored password in memory for session (None = not set)
        self.meta_password: Optional[str] = None
        # whether to remember password for session (always True here; no persistent storage)
    ...
    def _build_ui(self):
        ...
        settings_menu.add_checkbutton(label="Always encrypt metadata", variable=self.always_encrypt_var)
        settings_menu.add_command(label="Set metadata password...", command=self.set_metadata_password)
        settings_menu.add_command(label="Clear stored metadata password", command=self.clear_metadata_password)
        # optional: toggle remembering password in session (not persisted to disk)
        settings_menu.add_checkbutton(label="Remember password for session", variable=self.remember_password_var)
    ...
    def set_metadata_password(self):
        # Called from main thread (menu)
        p = simpledialog.askstring("Set metadata password", "Enter password to use for metadata encryption (will be kept in memory for this session):", show="*", parent=self.root)
        if p is None or p == "":
            messagebox.showinfo("Not set", "Password not set (empty). You can still encrypt per-save manually.")
            return
        self.meta_password = p
        messagebox.showinfo("Password set", "Metadata password set for this session. If 'Always encrypt metadata' is enabled, saves will use this password.")

    def clear_metadata_password(self):
        """Clear stored metadata password from this session (menu action)."""
        self.meta_password = None
        messagebox.showinfo("Cleared", "Stored metadata password cleared from session memory.")
        
    # Helper: ask password dialog from worker thread (schedules dialog on GUI thread and waits)
    def ask_password_blocking(self, prompt: str, title: str = "Password", allow_empty: bool = True) -> Optional[str]:
        """
        Schedule a simpledialog.askstring on the main thread and wait for the result.
        Returns:
          - str: entered password (can be empty string if allow_empty True and user left blank)
          - None: user cancelled (or not allowed empty and cancelled)
        """
        holder = {"value": None}
        evt = threading.Event()
        ...
        def ask():
            try:
                holder["value"] = simpledialog.askstring(title, prompt, show="*", parent=self.root)
            finally:
                evt.set()
        ...
        if val == "" and not allow_empty:
            # user submitted empty but empty not allowed -> treat as cancelled
            return None
        return val
    ...
    def save_as_fj4(self, lossy: bool):
        ...
        def worker():
            ...
            try:
                ...
                # Decide password according to settings:
                passwd = ""
                if self.always_encrypt_var.get():
                    # Always encrypt: need a password set (use stored or ask once)
                    if self.meta_password:
                        passwd = self.meta_password
                    else:
                        # ask once (do not allow empty)
                        p = self.ask_password_blocking(
                            "Always encrypt metadata is enabled. Enter password to encrypt metadata (this will be used for future saves in this session):",
                            title="Set metadata password",
                            allow_empty=False
                        )
                        if p is None:
                            # user cancelled -> abort save (because always_encrypt requires password)
                            self.root.after(0, lambda: messagebox.showinfo("Aborted", "Save aborted: metadata encryption is required but no password provided."))
                            pd.close()
                            return
                        passwd = p
                        # remember in session if chosen
                        if self.remember_password_var.get():
                            self.meta_password = passwd
                else:
                    # optional encryption: ask but allow empty => no encryption
                    p = self.ask_password_blocking(
                        "Enter password to encrypt metadata (leave empty = no encryption):",
                        title="Encrypt metadata",
                        allow_empty=True
                    )
                    if p is None:
                        # user cancelled -> treat as no encryption
                        passwd = ""
                    else:
                        passwd = p
        ...
        def worker():
            ...
            try:
                ...
                if raw[:4] == MAGIC:
                    ...
                    # Auto-decrypt metadata if it looks encrypted
                    if isinstance(meta, dict) and meta.get("_meta_cipher") == "fernet":
                        # 1) try stored session password first (no dialog)
                        decrypted = None
                        if self.meta_password:
                            try:
                                decrypted = decrypt_meta_dict(meta, self.meta_password)
                            except Exception:
                                decrypted = None
                        if decrypted is None:
                            # 2) attempt interactive prompt (allow user to cancel)
                            p = self.ask_password_blocking(
                                "Metadata is encrypted. Enter password to decrypt (cancel to abort):",
                                title="Decrypt metadata",
                                allow_empty=False
                            )
                            if p is None:
                                # user cancelled
                                self.root.after(0, lambda: messagebox.showinfo("Aborted", "Opening aborted: metadata is encrypted and password was not provided."))
                                pd.close()
                                return
                            try:
                                decrypted = decrypt_meta_dict(meta, p)
                                # if user wants to remember in session, store it
                                if self.remember_password_var.get():
                                    self.meta_password = p
                            except Exception as e:
                                self.root.after(0, lambda: messagebox.showerror("Decryption failed", f"Failed to decrypt metadata: {e}"))
                                pd.close()
                                return
                        meta = decrypted

                    # continue using (possibly decrypted) meta
                    self.loaded_metadata = meta
                    self.root.after(0, lambda: self._write_meta_text(self.loaded_metadata))

                    # rest of original processing (tape-mode / non-tape-mode) unchanged:
                    if meta.get("tape_mode"):
                        combined = parsed.get("payload")
                        ...
                        # Non-tape-mode (keep your original decompression/open logic)
                        ...
                        if parsed.get("compressed"):
                            try:
                                ...
                                if isinstance(stored_sha, (bytes, bytearray)):
                                    stored_sha = stored_sha.hex()

                                sha_ok = sha256_hex(decompressed) == stored_sha
                                self.loaded_metadata["_sha_ok"] = sha_ok
                                self.loaded_metadata["_sha_stored"] = stored_sha
                                ...
                                off = find_first_corrupt_offset_in_lz4_frame(parsed.get("payload")) if lz4frame is not None else None
                    ...
                    # raw file - try as TXT then PNG (keeps original behavior)
                    try:

...
class TinyAudioAdvanced:
    def __init__(self, codec_sr=16000, block_size=512, quant_levels=32,
        ...
        self.low_cut_hz = 150.0  # срез НЧ по умолчанию
    ...
    def highpass_filter(self, audio):
        if self.low_cut_hz <= 0:
            return audio
        nyq = 0.5 * self.codec_sr
        normal_cutoff = self.low_cut_hz / nyq
    ...
    def compress(self, wav_file, out_file):
        ...
        # --------- Реальный срез низких частот ---------
        audio = self.highpass_filter(audio)

        # --------- Нормализация после фильтра ---------
    ...
    def decompress(self, in_file):
        ...
        dct_blocks = q.astype(np.float32) / (self.quant_levels // 2 - 1) * maxv
...
class AudioPlayer:
    ...
    def _thread(self):
        chunk = 1024
        with sd.OutputStream(samplerate=self.sr, channels=1, dtype='float32', device=self.device) as stream:
...
# ---- Codec params ----
quality_slider = tk.Scale(root, from_=4, to=64, orient="horizontal", label="Качество")
...
tk.Checkbutton(root, text="Частотная нормализация", variable=freq_norm_var).grid(row=4, column=2)

# ---- Low cut slider ----
tk.Label(root, text="Срез низких частот (Hz):").grid(row=5, column=0)
lowcut_slider = tk.Scale(root, from_=0, to=200, orient="horizontal")
lowcut_slider.set(150)
lowcut_slider.grid(row=5, column=1, columnspan=2)

# ---- Output device ----
tk.Label(root, text="Выходное устройство:").grid(row=6, column=0)
devices = get_output_devices()
device_var = tk.StringVar(value=f"{devices[0][0]}: {devices[0][1]}")
tk.OptionMenu(root, device_var, *[f"{i}: {n}" for i, n in devices]).grid(row=6, column=1)
...
duration_label.grid(row=8, column=0, columnspan=3)
...
def compress_action():
    ...
    codec.low_cut_hz = lowcut_slider.get()
    codec.compress(entry_wav.get(), entry_txt.get())
    messagebox.showinfo("Готово", "Файл сжат (DCT + LZMA)")
...
def load_and_play():
    def worker():
        ...
        duration_label.config(text=f"Длительность: {len(decoded_audio)/decoded_sr:.2f} сек")
        player.play()
        update_slider()
...
# ---- Buttons ----
tk.Button(root, text="Сжать WAV → TinyAudio", command=compress_action).grid(row=9, column=0)
tk.Button(root, text="Декодировать и воспроизвести", command=load_and_play).grid(row=9, column=1)
tk.Button(root, text="💾 Сохранить WAV", command=save_decoded).grid(row=9, column=2)
tk.Button(root, text="⏮", command=player.rewind).grid(row=10, column=0)
tk.Button(root, text="▶", command=lambda: [player.play(), update_slider()]).grid(row=10, column=1)
tk.Button(root, text="⏸", command=player.pause).grid(row=10, column=2)
tk.Button(root, text="⏹", command=lambda: [player.stop(), slider.set(0)]).grid(row=11, column=0)
tk.Button(root, text="⏭", command=player.forward).grid(row=11, column=1)

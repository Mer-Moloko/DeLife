...
class TinyAudioAdvanced:
    ...
    def set_quality(self, q):
        self.quant_levels = max(4, min(q, 64))
    ...
    def compress(self, wav_file, out_file):
        ...
        if self.freq_norm:
            ...
            dct_blocks = dct_blocks / band_energy

        maxv = np.max(np.abs(dct_blocks), axis=1, keepdims=True) + 1e-9
        q = np.round(dct_blocks / maxv * (self.quant_levels // 2 - 1)).astype(np.int8)
    ...
    def decompress(self, in_file):
        ...
        self.freq_norm = bool(data.get('freq_norm', True))

        dct_blocks = (q.astype(np.float32) /
                      (self.quant_levels // 2 - 1)) * maxv
...
tk.Button(root, text="Выбрать",
          command=lambda: entry_wav.delete(0, tk.END) or entry_wav.insert(
              0, filedialog.askopenfilename(filetypes=[("WAV files", "*.wav")]))
          ).grid(row=0, column=2)
...
tk.Button(root, text="Выбрать",
          command=lambda: entry_txt.delete(0, tk.END) or entry_txt.insert(
              0, filedialog.asksaveasfilename(defaultextension=".npz"))
          ).grid(row=1, column=2)
...
tk.OptionMenu(root, device_var,
              *[f"{i}: {n}" for i, n in devices]).grid(row=4, column=1)
...
def compress_action():
    try:
        codec.set_codec_sr(int(codec_sr_entry.get()))
    except ValueError:
        messagebox.showerror("Ошибка", "Неверная частота")
        return

    codec.set_quality(quality_slider.get())
    codec.set_normalize(bool(normalize_var.get()))
    codec.set_freq_norm(bool(freq_norm_var.get()))
    codec.compress(entry_wav.get(), entry_txt.get())
    messagebox.showinfo("Готово", "Файл сжат")
